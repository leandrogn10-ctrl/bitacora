#!/bin/bash
# Fetches XcodeGen (no brew, no sudo) and generates Bitacora.xcodeproj from project.yml.
# Run:  bash ~/Projects/bitacora/ios-bitacora/setup-xcode.sh
set -u
cd "$(dirname "$0")" || { echo "cannot cd"; exit 1; }
echo "[1/4] building the web payload from ../index.html..."
python3 build-bitacora.py || { echo "BUNDLE FAILED"; exit 1; }
...
echo "[3/4] generating Bitacora.xcodeproj..."
"$BIN" generate --spec project.yml || { echo "GENERATE FAILED"; exit 1; }
echo "[4/4] result:"
ls -ld Bitacora.xcodeproj && echo "" && echo "SUCCESS."
