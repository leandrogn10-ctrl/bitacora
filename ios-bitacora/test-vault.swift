/*  test-vault.swift — falsifies VaultRule. Pure Foundation on purpose (no UIKit, no WebKit,
    no simulator, no signing): the rule that decides whether the logbook may be replaced has
...
func state(items: Int, modified: Double) -> Data {
    let arr = (0..<items).map { ["id": "i\($0)", "title": "t\($0)"] }
    return try! JSONSerialization.data(withJSONObject: ["items": arr, "lastModified": modified])
}
...
@main struct VaultGate {
    static func main() {
        ...
        ok(VaultRule.summarize(state(items: 46, modified: 1_700_000_000_000))
            == StateSummary(lastModified: 1_700_000_000_000, itemCount: 46), "reads count + lastModified")
        ok(VaultRule.summarize(Data("not json".utf8)) == nil, "garbage is not a state")
        ok(VaultRule.summarize(Data(#"{"lastModified":1}"#.utf8)) == nil, "no items array is not a state")
        ...
        ok(VaultRule.summarize(Data(#"{"items":[]}"#.utf8))?.lastModified == 0, "missing lastModified reads as epoch 0, not now")
        ...
        if case .heldShrink(let was, let now) = VaultRule.decide(incoming: wipe, current: full) {
            ok(was == 46 && now == 0, "a newer EMPTY save is held, not accepted (46→0)")
        ...
        ok(VaultRule.decide(incoming: state(items: 47, modified: 1_700_000_000_000), current: full) == .accepted,
           "an equal timestamp with more items is accepted (a same-ms save is not a regression)")
        ...
        ok(VaultRule.decide(incoming: Data("{".utf8), current: full) == .rejectedInvalid("payload is not a Bitácora state (no items array)"),
           "a truncated payload never touches the primary")

        print("\n── collapse: where the line sits ──")
        ok(!VaultRule.isCollapse(was: 46, now: 45), "deleting one title is an edit, not a collapse")
        ok(!VaultRule.isCollapse(was: 46, now: 41), "deleting five is still an edit")
        ok(VaultRule.isCollapse(was: 46, now: 40),  "losing six at once is held")
        ...
        ok(VaultRule.isCollapse(was: 6, now: 1), "6→1 is held by the proportional arm")
        ok(!VaultRule.isCollapse(was: 2, now: 1), "2→1 is too small a library to judge; allowed")
